...
DEFAULTS = {
    ...
    "fuel_in_transit": "", "transit_eta": "",
    "consumption_day": "", "consumption_norm": "280",
    ...
    "rail_level": "1",
    "azs_ok": "", "azs_limit": "", "azs_empty": "", "queues": "0", "dry_zone": "",
    ...
    "price_95": "120", "price_dt": "",
    "price_wholesale": "", "price_shadow": "", "price_national": "",
...
def load_data(date=None):
    ...
    if row:
        d = dict(DEFAULTS)
        d.update(json.loads(row["payload"]))
        return d
...
def save_data(d):
    ...
    conn = get_db()
    conn.execute(
        "INSERT INTO daily_data(date, payload) VALUES(?,?) ON CONFLICT(date) DO UPDATE SET payload=excluded.payload",
...
def safe_pct(fact, norm):
    try:
        f, n = float(fact), float(norm)
        return round(f / n * 100) if n else None
    except: return None

def safe_div(a, b):
    try:
        a2, b2 = float(a), float(b)
        return round(a2 / b2, 1) if b2 else None
    except: return None
...
def dashboard():
    ...
    soc_defs = [
        ...
        ("police","Полиция / МВД"),    ("school","Школьные маршруты"),
        ("hospital","Больницы"),       ("generator","Рез. генераторы (ДТ)"),
        ("water","Водоснабжение"),     ("heat","Теплоснабжение"),
        ("food","Прод. логистика"),
    ]
    d["soc_items"] = [{"key": k, "label": l,
        "fact": d.get(f"soc_{k}_fact",""),
        "norm": d.get(f"soc_{k}_norm", 1),
        "pct":  safe_pct(d.get(f"soc_{k}_fact",""), d.get(f"soc_{k}_norm", 1))}
        for k, l in soc_defs]
    risk_labels = {"0":"Безопасен", "1":"Повышенный риск", "2":"Опасен"}
    rail_labels  = {"0":"Не задействовано", "1":"Частичная", "2":"Полная"}
    d["route1_risk_label"] = risk_labels.get(str(d.get("route1_risk","0")), "")
    d["route2_risk_label"] = risk_labels.get(str(d.get("route2_risk","0")), "")
    d["rail_label"] = rail_labels.get(str(d.get("rail_level","1")), "")
...
def add_log():
    conn = get_db()
    conn.execute(
        "INSERT INTO log_entries(entry_date, action, responsible, status, created_at) VALUES(?,?,?,?,?)",
...
def update_log_status():
    conn = get_db()
    conn.execute("UPDATE log_entries SET status=? WHERE id=?",
...
if __name__ == "__main__":
    ...
    import socket
    print("\n" + "="*50)
    print("  ДАШБОРД — УПРАВЛЕНИЕ БЕНЗИНОМ")
    print(f"  Локально:  http://localhost:5000")
    try:
        ip = socket.gethostbyname(socket.gethostname())
        print(f"  По сети:   http://{ip}:5000")
    except: pass
    print("="*50+"\n")
